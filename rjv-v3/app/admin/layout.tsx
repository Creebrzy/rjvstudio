...
import { LayoutDashboard, Calendar, Settings, Users, LogOut, Menu, Image } from 'lucide-react';
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (checking) return (
    <div className="min-h-screen bg-void flex items-center justify-center">
      <div className="font-display text-2xl text-gold">RJV MEDIA LAB<span className="text-white">.</span></div>
    </div>
  );
    ...
    <div className="min-h-screen bg-void flex">
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-void-2 border-r border-white/5 flex flex-col transition-transform md:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 border-b border-white/5">
          <Link href="/admin" className="font-display text-xl text-white">RJV MEDIA LAB<span className="text-gold">.</span></Link>
      ...
      <div className="flex-1 md:ml-64">
        <header className="bg-void-2 border-b border-white/5 h-14 flex items-center px-6 md:hidden">
          ...
          <span className="font-display text-xl text-white ml-4">RJV MEDIA LAB<span className="text-gold">.</span></span>
        </header>
